...
$unlockCode = strtoupper(post_value('unlock_code', 'code'));
$nis = post_value('nis');

if ($unlockCode === '' || $nis === '') {
    json_fail(422, 'Unlock code dan NIS wajib diisi');
}
...
if (!$codeStmt) {
    json_fail(500, 'Gagal menyiapkan query unlock code');
}

$codeStmt->bind_param('s', $unlockCode);
$codeStmt->execute();
...
if (!$code) {
    json_fail(401, 'Unlock code tidak valid');
}
...
if (!empty($expiredAt)) {
    ...
    if ($expiry < $now) {
        json_fail(401, 'Unlock code sudah expired');
    }
}
